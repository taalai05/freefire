...
def get_player_info(Id):
    ...
    try:
        ...
        print(
            f"HTTP Error: {errh} - Response: {errh.response.text if errh.response is not None else 'No response body'}")
...
def region():
    ...
    try:
        ...
        return jsonify({
                           "message": "Error: Upstream API returned non-JSON response. UID might not be found or API structure changed."}), 200
